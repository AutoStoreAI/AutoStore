...
function Cart({ onBack, onApprove }: { onBack: () => void; onApprove: () => void }) {
  const [items, setItems] = useState(() => cart.map((product, index) => ({ product, quantity: index === 0 ? 2 : 1 })));
  const [isEditing, setIsEditing] = useState(false);
  const units = items.reduce((total, item) => total + item.quantity, 0);
  const total = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
  const formatPrice = (price: number) => `${price.toFixed(2).replace(".", ",")} €`;
  const changeQuantity = (name: string, amount: number) => setItems(current => current.map(item => item.product.name === name ? { ...item, quantity: Math.max(1, item.quantity + amount) } : item));
  const removeItem = (name: string) => setItems(current => current.filter(item => item.product.name !== name));

  return <main className="app-shell screen-enter"><header><Back onClick={onBack} /><Logo /><span className="header-spacer" /></header><section className="intro cart-intro"><div className="eyebrow"><i />Carrito inteligente</div><h1>Tu compra está lista.</h1><p>Hemos reunido los productos que probablemente necesitas.</p></section>{items.length ? <><div className="cart-list">{items.map(({ product, quantity }, index) => <article className="cart-item" key={product.name}><span className="product-icon large" style={{ background: product.color }}>{product.icon}</span><div><strong>{product.name} <span className="quantity">× {quantity}</span></strong><span>{index === 0 ? "Tu marca habitual" : "Según tu rutina"}</span>{isEditing && <div className="item-editor"><button type="button" onClick={() => changeQuantity(product.name, -1)} aria-label={`Reducir ${product.name}`}>−</button><b>{quantity}</b><button type="button" onClick={() => changeQuantity(product.name, 1)} aria-label={`Aumentar ${product.name}`}>+</button><button type="button" className="remove-item" onClick={() => removeItem(product.name)}>Eliminar</button></div>}</div><b>{formatPrice(product.price * quantity)}</b></article>)}</div><div className="total"><span><b>{units} productos</b><small>Total estimado</small></span><strong>{formatPrice(total)}</strong></div><div className="cart-actions">{isEditing ? <button className="secondary-action" onClick={() => setIsEditing(false)}>✓ Guardar cambios</button> : <button className="secondary-action" onClick={() => setIsEditing(true)}>✎ Editar carrito</button>}<button className="primary" onClick={onApprove}>✓&nbsp; Aprobar compra <span>→</span></button></div></> : <div className="empty-cart"><strong>Tu carrito está vacío.</strong><button className="secondary-action" onClick={onBack}>Volver a productos</button></div>}<p className="secure">Demo: no se realizará ningún cargo.</p></main>;
}
